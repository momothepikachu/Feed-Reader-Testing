    ...
    */
    ...
    describe('Initial Entries', function(){
        ...
        it('have at least one entry wihtin the feed container', function(done){
            expect(feedContainer.children.length>=1).toBe(true);
            done();
        });
    });
    ...
    describe('New Feed Selection', function(){
        ...
        */
        let oldFeed = document.querySelector('.feed');
        beforeEach(function(done){
            loadFeed(Math.floor(Math.random() * Math.floor(3))+1, function(){
                done();
            });
        });        
        it('is different than the initial entries', function(done){
            expect(document.querySelector('.feed')).toEqual(oldFeed);
            done();
        })           
    });
